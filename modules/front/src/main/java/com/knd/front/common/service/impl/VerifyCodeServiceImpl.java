...
public class VerifyCodeServiceImpl extends ServiceImpl<VerifyCodeMapper, VerifyCode> implements IVerifyCodeService {
    ...
    public Result getVerificationCode(CodeDto codeDto) {
        ...
        //检查改手机号是否在1分钟内发送过验证码
        //根据手机号，获取最新的验证发送时间
        log.info("getVerificationCode codeDto:{{}}",codeDto);
        ...
        log.info("getVerificationCode codeDto:{{}}",v);
        if (v != null) {
            //有数据
            //比较时间
        ...
        log.info("getVerificationCode user:{{}}",user);
        //注册过的不允许再发注册验证码
        ...
        log.info("getVerificationCode getByMobile:{{}}",getByMobile);
