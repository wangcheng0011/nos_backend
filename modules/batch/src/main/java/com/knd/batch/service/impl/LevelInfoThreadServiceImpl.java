...
import com.knd.batch.util.TrainWeek;
import com.knd.batch.util.WeekData;
...
public class LevelInfoThreadServiceImpl implements LevelInfoThreadService {
    ...
    //每周可弥补的最大天数
    private static final int  weekBufferDay = 4;


    @Override
    public void doLevelInfoThread(User u, LocalDate yesterday) {
        log.info("用户 "+ u.getId() +" 的具体信息："+u.toString()+" ,,,,,训练周期开始时间是="+u.getTrainPeriodBeginTime()+"||");
        ...
        try {
            ...
            if (StringUtils.isEmpty(u.getTrainPeriodBeginTime())) {
                ...
                if (!ld.isEmpty()) {
                    //不空,【说明有完成训练】
                    log.info("75行，无训练周期但是有完成的课程训练信息==" + u.getId());
                    this.noTrainBeginInUser(u, ld);
                } else {
                    //空的，没有完成过课程训练，结束这一次循环
                    log.info("79行，无训练周期,没有完成过课程训练，结束这一次循环==" + u.getId());
                }
            } else {
                //不空
                //根据用户id,获取最新的训练周期信息
                ...
                if (!yesterday.isAfter(trainPeriodEndTime)) {
                    log.info("89行，昨天日期在有记录的最新范围内==" + u.getId());
                    //是，昨天日期在有记录的最新范围内
...
//                        System.out.println(u.getId() + "用户有训练周期开始时间，但是已经宕机一年以上了，目前不做处理");
    ...
    private void noTrainBeginInUser(User u, List<Date> ld) {
        ...
        if (minDate.isEqual(yesterday)) {
            log.info("用户 " + u.getId() + "说明昨天是用户第一次完成课程训练的日期,136行");
            ...
            //判断昨天日期是否在该训练周期范围里？
            if (!yesterday.isAfter(trainPeriodEndTime)) {
                log.info("用户 " + u.getId() + "用户没有训练周期开始时间,但是在昨天之前就有课程训练数据，宕机计算，一年之内，143行");
                //是
...
//                System.out.println(u.getId() + "用户没有训练周期开始时间，但是在一年之前就已经有课程训练数据了，也就是说宕机一年以上，目前不做处理");
                log.info("用户 " + u.getId() + "用户没有训练周期开始时间,但是在一年之前就已经有课程训练数据了,宕机不执行，152行");
            }
        }
    }
    ...
    private void yesterdayHaveCourseAtFirst(User u, LocalDate minDate, LocalDate trainPeriodEndTime, String trainDayCount) {
        ...
        log.info("170行，用户 " + u.getId() + "更新user表");
        //更新user表
    ...
    private void downtimeAtNoTrainBegin(User u, LocalDate minDate, LocalDate trainPeriodEndTime) {
        ...
        u.setLastModifiedDate(LocalDateTime.now());
        u.setLastModifiedBy("等级更新程序");
        log.info("192行，用户 " + u.getId() + "更新user表");
        //更新user表
        userMapper.updateById(u);
        //user_train_level表新增
        //当前训练周期id
        ...
        log.info("201行，用户 " + u.getId() + "获取每一周的开始和结束日期，数据："+lw);
        //当前星星总数
        int starCount = 0;
    ...
    private int dostars(User u, List<TrainWeek> lw, int starCount, String userTrainLevelId) {
        ...
        for (TrainWeek w : lw) {
            ...
            log.info("257行，用户 " + u.getId() + "计算的这一周数据，数据："+w.toString()+" ,完成的天数："+traindaycount);
            ...
            if (!yesterday.isBefore(w.getEndDate())) {
                ...
                if (traindaycount > 3) {
                    ...
                    //不满勤4天，
                    //计算需要消耗的缓冲天数
                    int useBuffer = weekBufferDay - traindaycount;
                    //计算缓冲剩余天数
    ...
    private UserLevelInfo insertUserLevelInfo(User u, String sort, String beginDate, String endDate, String starFlag, String traindaycount,
        ...
        info.setDeleted("0");
        log.info("用户 " + u.getId() + " 向表UserLevelInfo插入一条新的数据,代码331行，这条数据内容是：" + info.toString() + "，，，id是：" + info.getId());
    ...
    private void doLevel(User u, int starCount, UserTrainLevel t, String LevelWord) {
        ...
        if (!level.equals(LevelWord)) {
            ...
            u.setLastModifiedDate(LocalDateTime.now());
            log.info("346行，用户 " + u.getId() + " 星星数量有变化，更新user表，346行");
            userMapper.updateById(u);
            t.setTrainLevel(level);
            t.setLastModifiedDate(LocalDateTime.now());
            log.info("350行，用户 " + u.getId() + " 星星数量有变化，更新 UserTrainLevel 表，350行，id是"+t.getId());
   ...
   */
    ...
    private String doLevel2(int starCount, UserTrainLevel t, String LevelWord) {
        ...
        if (!level.equals(LevelWord)) {
            ...
            t.setTrainLevel(level);
            t.setLastModifiedDate(LocalDateTime.now());
            log.info("用户 " + t.getUserId() + " 星星数量有变化，只需要记录到历史等级信息表，367行，id是"+t.getId());
            userTrainLevelMapper.updateById(t);
        }
        return level;
    }
    ...
    private void haveTrainBeginInUserInPeriod(User u, UserTrainLevel t) {
        ...
        log.info("用户 " + u.getId() + " 根据用户id获取最新的周日期信息。379行," + info.toString() + ",,,id是" + info.getId());
        //将周开始和结束日期字符串转成日期格式
        ...
        if (!yesterday.isBefore(w.getBeginDate()) && !yesterday.isAfter(w.getEndDate())) {
            ...
            log.info("388行，用户 " + u.getId() + " 昨天日期在该周日期中,训练条数为："+traindaycount);
            this.yesterdayInWeek(u, w, info, traindaycount, t);
        } else {
            //不在
            log.info("用户 " + u.getId() + " 。392行，info.getLastModifiedDate().toLocalDate()=" + info.getLastModifiedDate().toLocalDate() + "," +
                    "yesterday=" + yesterday);
            //判断该周的最新修改日期是否等于昨天日期？
            if (info.getLastModifiedDate().toLocalDate().isEqual(yesterday)) {
                log.info("用户 " + u.getId() + " 进入下一周。396行");
                //是，进入下一周啦,【即昨天是新周开始的第一天】
                this.intoNextWeek(u, info);
            } else {
                log.info("用户 " + u.getId() + " 进入下一周,但是该周的最新修改日期不等于昨天日期，进入宕机计算，400行");
    ...
    private UserLevelInfo getUserLevelInfo(User u, UserTrainLevel t) {
        ...
        //是否为空
        if (info == null) {
            log.info("用户 " + u.getId() + " ，UserLevelInfo空的，数据库被改动了,全部被删除了,直接数据初始化即可，给user_level_info表新增一条数据,然后等级信息都更新为新手,414行");
            //空的，数据库被改动了,全部被删除了
    ...
    private void yesterdayInWeek(User u, TrainWeek w, UserLevelInfo info, int traindaycount, UserTrainLevel t) {
        ...
        log.info("445行，用户 " + u.getId() + " 获取周开始的第4天日期,为："+no4Day.toString());
        //判断昨天日期是否是该周的前3天任意一天？
        if (yesterday.isBefore(no4Day)) {
            //是
            log.info("449行，用户 " + u.getId() + " 昨天日期是该周的前3天任意一天");
            //【user_level_info表更新】
            info.setTrainDayCount(traindaycount + "");
            info.setLastModifiedDate(LocalDateTime.now());
        } else {
            log.info("454行，用户 " + u.getId() + " 昨天日期不是该周的前3天任意一天");
            //不是
            ...
            if (traindaycount > 3) {
                ...
                if (yesterday.isEqual(w.getEndDate())) {
                    //是
                    //判断剩余缓冲时间能否抵消
                    if (surplusBuffer - (weekBufferDay - traindaycount) >= 0) {
                        //能
                        info.setBufferDayCount((weekBufferDay - traindaycount) + "");
                    } else {
                        //不能
                        //判断星星总数是否大于0
                        if (starCount > 0) {
        ...
        log.info("用户 " + u.getId() + " 向表UserLevelInf更新一条新的数据,代码497行，这条数据内容是：" + info.toString() + "，，，id是：" + info.getId());
        //更新
    ...
    private void downtimeBeforeTrainEnd(User u, UserLevelInfo info, TrainWeek w, UserTrainLevel t) {
        ...
        log.info("用户 " + u.getId() + " 获取每一周需要计算的宕机日期。521行，"+lw);
...
//        int surplusBuffer = 7 * 7 - sab.getBufferDayCount() + weekData.getTempBuffer();
        ...
        for (TrainWeek w : lw) {
            ...
            log.info("581行，用户 " + u.getId() + "计算的这一周数据，数据："+w.toString()+" ,完成的天数："+mtraindaycount);
            //判断昨天日期 是否 不在该周内 或者 刚好是周结束日期
            if (!yesterday.isBefore(w.getEndDate())) {
                //是,如果不满勤 会 扣一颗星星
                if (mtraindaycount > 3) {
                    ...
                    //不满勤4天，
                    //计算需要消耗的缓冲天数
                    int useBuffer = weekBufferDay - mtraindaycount;
                    //计算缓冲剩余天数
                    int temp = surplusBuffer - useBuffer;
                    if (temp >= 0) {
                        //缓冲天数足够抵消，不扣星星
            ...
            if (isAdd) {
                ...
                minfo.setDeleted("0");
                log.info("用户 " + u.getId() + " 向表UserLevelInfo插入一条新的数据,代码661行，这条数据内容是：" + minfo.toString() + "，，，id是：" + minfo.getId());
                baseMapper.insert(minfo);
            } else {
                //更新
                minfo.setId(infoId);
                log.info("用户 " + u.getId() + " 向表UserLevelInfo更新一条数据,代码666行，这条数据内容是：" + minfo.toString() + "，，，id是：" + minfo.getId());
    ...
    */
    ...
    private void doLastTrainCycle(User u, UserTrainLevel t, UserLevelInfo info, List<TrainWeek> lw, int starCountTemp,
        ...
        for (TrainWeek mw : lw) {
            ...
            if (isAdd) {
                ...
                minfo.setCreateDate(LocalDateTime.now());
                minfo.setId(UUIDUtil.getShortUUID());
                log.info("用户 " + u.getId() + " 向表UserLevelInfo插入一条新的数据,代码898行，这条数据内容是：" + minfo.toString() + "，，，id是：" + minfo.getId());
                baseMapper.insert(minfo);
            } else {
                //更新
                minfo.setCreateDate(info.getCreateDate());
                minfo.setId(infoId);
                log.info("用户 " + u.getId() + " 向表UserLevelInfo更新一条数据,代码904行，这条数据内容是：" + minfo.toString() + "，，，id是：" + minfo.getId());
                baseMapper.updateById(minfo);
                //下一周将需要新增
                //设为新增
                isAdd = true;
    ...
    private UserTrainLevel insertTrainLevel(User u, String userTrainLevelId, String trainLevel, String trainCycleBegin,
        ...
        t2.setDeleted("0");
        log.info("用户 " + u.getId() + " 向表 UserTrainLevel 插入一条新的数据,代码940行，这条数据内容是：" + t2.toString() + "，，，id是：" + t2.getId());
